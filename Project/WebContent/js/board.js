...
function post_data_print(block) {
    ...
    let post_list = document.querySelectorAll(".j_board_list tbody");
    post_list.forEach(function (item) {
        item.remove();
    })

    // 게시글 출력 공간
    let notice_board = document.querySelector("#j_board_list");
    // 출력 첫 페이지 번호
    ...
    for (let i = start; i >= 1 && i > start - page_num; i--) {
        ...
        post.className = "j_board_top";
        post.className = "data_row";

        let classname = ["j_list_num", "j_list_title", "j_list_writer", "j_list_date", "j_list_count"]
